	...
	initLinkage : function(dataSrc, sValue, nLevel) {
		...
		if(this.need_ajax)
		{
			...
			if(parent_id_str !== "")
			{
				...
				this.myAjax = new Ajax(url, {method: 'get', evalScripts: true, encoding:this.encoding, onComplete:this.set_children_list_select_data.bind(this, [dataSrc, sValue, nLevel])}).request();
	...
	init_first_select : function()
	{
		if(this.need_ajax)
		{
			if(global_arr_response_record[0] == "undefined" || global_arr_response_record[0] == null)
			{
				...
				this.myAjax = new Ajax(url, {method: 'get', evalScripts: true, encoding:this.encoding, onComplete:this.set_first_select_data.bind(this)}).request();
				if(this.debug)
				{
					$("debug_content").innerHTML += "请求地址：" + url + "<br  />";
				}
